...
def function(data_object):
    ...
    response = requests.get('http://localhost:5000/recruiter')
    options = response.json()
    print(options)
    location = options['location']
    jobPosition = options['job_position']
    jobDescription = options['job_description']
    domain = options['domain']
    if location != "":
        df_location = df[df['location'].str.contains(location, case=True)]
    else:
        df_location = df[df['location'].str.contains("", case=False)]
    df_location_role= df_location[df_location['description'].str.contains(jobPosition, case=True)]
    df_location_role
    ...
    filtered_df["email"] = filtered_df.apply(lambda row: generate_email(row, jobDescription), axis=1)
